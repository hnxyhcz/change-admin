...
// 根据类型查询字典数据（精简)列表
export const simpleDictDataByType = (dictType: string) => {
  return request.get({ url: `/system/dict-data/list-all-simple/${dictType}` });
};

// 查询字典数据列表
export const getDictDataPageApi = (params: DictDataPageReqVO) => {
  return request.getPage<DictDataVO, DictDataPageReqVO>({
...
export const updateDictDataApi = (data: DictDataVO) => {
  return request.post({ url: '/system/dict-data/update', data });
};

// 删除字典数据
export const deleteDictDataApi = (id: number) => {
  return request.post({ url: '/system/dict-data/delete?id=' + id });
};
// 导出字典类型数据
