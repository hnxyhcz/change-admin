...
export const getDictTypePageApi = (params: DictTypePageReqVO) => {
  return request.getPage<DictTypeVO, DictTypePageReqVO>({
...
export const updateDictTypeApi = (data: DictTypeVO) => {
  return request.post({ url: '/system/dict-type/update', data });
};

// 删除字典
export const deleteDictTypeApi = (id: number) => {
  return request.post({ url: '/system/dict-type/delete?id=' + id });
};
