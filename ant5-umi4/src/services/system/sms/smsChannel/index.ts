...
import type { SmsChannelVO, SmsChannelPageVO } from './types';

const request = useAxios();

// 查询短信渠道列表
export const getSmsChannelPageApi = (params: SmsChannelPageVO) => {
  return request.getPage<SmsChannelVO, SmsChannelPageVO>({
...
export const updateSmsChannelApi = (data: SmsChannelVO) => {
  return request.post({ url: '/system/sms-channel/update', data });
};

// 删除短信渠道
export const deleteSmsChannelApi = (id: number) => {
  return request.post({ url: '/system/sms-channel/delete?id=' + id });
};
