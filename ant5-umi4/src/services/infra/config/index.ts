...
import type { ConfigPageVO, ConfigReqVO, ConfigVO } from './types';

const request = useAxios();

// 查询参数列表
export const getConfigPageApi = (params: ConfigPageVO) => {
  return request.getPage<ConfigVO, ConfigPageVO>({
...
export const updateConfigApi = (data: ConfigVO) => {
  return request.post({ url: '/infra/config/update', data });
};

// 删除参数
export const deleteConfigApi = (id: number) => {
  return request.post({ url: '/infra/config/delete?id=' + id });
};

// 导出参数
export const exportConfigApi = (params: ConfigReqVO) => {
  return request.downloadBlob({ url: '/infra/config/export', params });
};
