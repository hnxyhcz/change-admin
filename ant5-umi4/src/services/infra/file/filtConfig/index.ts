...
import type { FileConfigReqVO, FileConfigVO } from './types';

const request = useAxios();

// 查询文件配置列表
export const getFileConfigPageApi = (params: FileConfigReqVO) => {
  return request.getPage<FileConfigVO, FileConfigReqVO>({
...
export const updateFileConfigMasterApi = (id: number) => {
  return request.post({ url: '/infra/file-config/update-master?id=' + id });
};
...
export const updateFileConfigApi = (data: FileConfigVO) => {
  return request.post({ url: '/infra/file-config/update', data });
};

// 删除文件配置
export const deleteFileConfigApi = (id: number) => {
  return request.post({ url: '/infra/file-config/delete?id=' + id });
};
